...
public:
    ...
    typedef std::function<void(IResource *_pResource, EventHandle _iEventHandle)> EventSignature;
...
protected:
    ...
    EventHandle m_iEventHandle;
    
    E_RESOURCE_TYPE m_eResourceType;
    E_CREATION_MODE m_eCreationMode;
    
    std::unordered_map<EventHandle, EventSignature> m_lListeners;
public:
    IResource(void);
    ...
    EventHandle AddEventListener(const EventSignature& _pListener);
    void RemoveEventListener(EventHandle _eventHandler);
