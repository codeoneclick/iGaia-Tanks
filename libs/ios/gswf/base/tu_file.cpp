...
// cause that to not be equal to "bytes".
{
	...
	int	bytes_to_read = gswf_math::imin(bytes, buf->m_.size() - buf->m_position);
	if (bytes_to_read)
	{
...
// Return the number of bytes actually written.
{
	...
	int	bytes_to_expand = gswf_math::imax(0, buf->m_position + bytes - buf->m_.size());
	if (bytes_to_expand)
	{
...
// number of bytes copied.
{
	...
	while (bytes_left)
	{
		int	to_copy = gswf_math::imin(bytes_left, BUFSIZE);
