...
{
	y = gswf_math::iclamp(y, 0, image->m_height - 1);
	int	x1 = x0 + xsize - 1;
	if (x1 >= image->m_width) {
		// clip, then extend.
...
// Copy RGB data from the specified column into the given buffer.
{
	...
	if ((x < 0) || (x >= image->m_width)) {
		assert(0);
		x = gswf_math::iclamp(x, 0, image->m_width - 1);
	}
...
// at (x, y).
{
	...
	p[x * 3 + 0] = gswf_math::iclamp(gswf_math::frnd(r), 0, 255);
	p[x * 3 + 1] = gswf_math::iclamp(gswf_math::frnd(g), 0, 255);
	p[x * 3 + 2] = gswf_math::iclamp(gswf_math::frnd(b), 0, 255);
}
...
// at (x, y).
{
	...
	p[x * 4	+ 0] = gswf_math::iclamp(gswf_math::frnd(r), 0, 255);
	p[x * 4	+ 1] = gswf_math::iclamp(gswf_math::frnd(g), 0, 255);
	p[x * 4	+ 2] = gswf_math::iclamp(gswf_math::frnd(b), 0, 255);
	p[x * 4	+ 3] = gswf_math::iclamp(gswf_math::frnd(a), 0, 255);
}
	...
	if(xscale < 1.0f) {
		...
		for (i = 0; i < tmp->m_width; ++i) {
			...
			for (j = left; j <= right; ++j) {
				...
				n = gswf_math::iclamp(j, 0, in_window_w - 1);
	...
	if (yscale < 1.0f) {
		...
		for (i = 0; i < out_height; ++i) {
			...
			for (j = left; j <= right; ++j) {
				weight = center - (float) j;
				weight = (*filter_function)(weight / fscale) / fscale;
				n = gswf_math::iclamp(j, 0, tmp->m_height - 1);
				contrib[i].push_back(CONTRIB(n, weight));
			}
		}
	} else {
