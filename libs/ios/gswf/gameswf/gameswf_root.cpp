...
namespace gameswf
{
	...
	void	root::set_display_viewport(int x0, int y0, int w, int h)
	{
		...
		m_pixel_scale = gswf_math::fmax(scale_x, scale_y);
	}
	...
	void	root::do_mouse_drag()		
	{
		...
		if (m_drag_state.GetBounds(&origBoundRect))
		{
			...
			worldMouse.m_x = gswf_math::fclamp(worldMouse.m_x, bounds.m_x_min, bounds.m_x_max);
			worldMouse.m_y = gswf_math::fclamp(worldMouse.m_y, bounds.m_y_min, bounds.m_y_max);
		}
	...
	{
		m_background_color.m_a = gswf_math::iclamp(gswf_math::frnd(alpha * 255.0f), 0, 255);
	}
