...
namespace gameswf
{
	...
	struct pointi
	{
		...
		{
			return gswf_math::imin(m_x, m_y) < gswf_math::imin(p.m_x, p.m_y);
		}
	};
	...
	// Fill the specified trapezoid in the software output buffer.
	{
		...
		for (int y = iy0; y < iy1; y++)
		{
			...
			float	f = (y - y0) / dy;
			int	xl = (int) ceilf(gswf_math::flerp(xl0, xl1, f));
			int	xr = (int) ceilf(gswf_math::flerp(xr0, xr1, f));
			
			xl = gswf_math::iclamp(xl, 0, s_rendering_box - 1);
			xr = gswf_math::iclamp(xr, 0, s_rendering_box - 1);
