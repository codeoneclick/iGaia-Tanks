...
namespace gameswf
{
	...
	void	morph2_character_def::display(character* inst)
	{
		...
		for (i = 0; i < m_line_styles.size(); i++)
		{
			...
			ls.m_width = (Uint16)gswf_math::frnd(gswf_math::flerp(ls1.get_width(), ls2.get_width(), ratio));
		...
		for (i = 0; i < m_paths.size(); i++) 
		{
			...
			p.m_ax = gswf_math::flerp(p1.m_ax, m_shape2.get_paths()[n].m_ax, ratio);
			p.m_ay = gswf_math::flerp(p1.m_ay, m_shape2.get_paths()[n].m_ay, ratio);
			...
			{
				p.m_edges[j].m_cx = gswf_math::flerp(p1.m_edges[j].m_cx, m_shape2.get_paths()[n].m_edges[k].m_cx, ratio);
				p.m_edges[j].m_cy = gswf_math::flerp(p1.m_edges[j].m_cy, m_shape2.get_paths()[n].m_edges[k].m_cy, ratio);
				p.m_edges[j].m_ax = gswf_math::flerp(p1.m_edges[j].m_ax, m_shape2.get_paths()[n].m_edges[k].m_ax, ratio);
				p.m_edges[j].m_ay = gswf_math::flerp(p1.m_edges[j].m_ay, m_shape2.get_paths()[n].m_edges[k].m_ay, ratio);
				k++;
