...
namespace gameswf
{
	...
	void string_slice(const fn_call& fn)
	{
		...
		start = gswf_math::iclamp(start, 0, len);
		end = gswf_math::iclamp(end, start, len);
	...
	void string_substr(const fn_call& fn)
	{
		...
		start = gswf_math::iclamp(start, 0, utf8_len);

		if (fn.nargs >= 2)
		{
			len = fn.arg(1).to_int();
			len = gswf_math::iclamp(len, 0, utf8_len);
		}
	...
	void string_substring(const fn_call& fn)
	{
		...
		if (fn.nargs >= 2)
		{
			...
			end = gswf_math::iclamp(end, 0, utf8_len);
		}
